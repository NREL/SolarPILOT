...
void SPFrame::CreateSimulationsParametricsTab( wxScrolledWindow *param){
	...
	//_par_button->GetToolTip()->SetDelay(_tool_tip_delay);
	...
	//Data options
	//_user_save_field_data = new wxCheckBox(_user_par_panel, wxID_ANY, wxT("Save heliostat performance data")),
	//_user_save_system_summary = new wxCheckBox(_user_par_panel, wxID_ANY, wxT("Save system summary information"));
	InputControl
	...
	try{
		...
		if(dlg->ShowModal() == wxID_OK){
			...
			for(int i=0; i<(int)vals.size(); i++){
				...
				_par_data.addVar( _variables._varptrs.at(vals[i].ToStdString() ) ); // getVarByString(_variables, vals[i].ToStdString()) );
				nnew++;
			}
...
void SPFrame::OnParRemoveVar( wxCommandEvent &WXUNUSED(event)){
	...
	try{
		...
		{
			/*wxString name = mPSimInf->Variables[idx].VarName;
			if (mPSimInf->Linkages.Index(name)>=0)
				mPSimInf->Linkages.Remove(name);
		
			if (mPSimInf->Linkages.Count() < 2)
				mPSimInf->Linkages.Clear();
			*/
...
void SPFrame::OnParEditVar( wxCommandEvent &WXUNUSED(event)){
	try{
		...
		else
		{
			...
			if( dlg->ShowEditValuesDialog(wxT("Edit parametric values"), values, &_par_data.at(idx)) ){	//modifies the 'values' array
...
void SPFrame::OnParEditLinkages( wxCommandEvent &WXUNUSED(event)){
	...
	try{
		...
		//par_values_dialog *dlg = new par_values_dialog((wxWindow*)NULL, wxID_ANY, _install_dir+"exelib\\images\\", wxT("Choose Linked Parametric Variables"));
		//dlg->ShowFixedDomainDialog(wxT("Choose Linked Parametric Variables"), labels, contexts, cur_values);
...
void SPFrame::OnUserParSimulate( wxCommandEvent &WXUNUSED(event)){
	...
	*/
	try{
	    ...
	    bool save_summary = _variables.par.upar_save_summary.val; //_par_save_system_summary->GetValue();
	    bool save_field = _variables.par.upar_save_helio.val; //_par_save_field_data->GetValue();
		...
		//Check the values in the table and make sure they're reasonable
	


		/*bool save_field = _user_save_field_data->GetValue();
		bool save_summary = _user_save_system_summary->GetValue();*/

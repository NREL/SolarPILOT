...
void SPFrame::CreateLayoutPage(wxScrolledWindow *parent){
	...
	//sbs_1->Add(new wxStaticLine(parent, wxID_ANY, wxDefaultPosition, wxSize(1,1), wxHORIZONTAL), 1, wxEXPAND|wxALL, 5);
...
//	is_bounds_scaled->setDisabledSiblings("true", 0, (wxWindow*)0);
    {
    wxWindow* dsibs[] = {max_scaled_rad, min_scaled_rad};
    is_bounds_scaled->setDisabledSiblings("false", 2, dsibs);
    }
//	is_bounds_fixed->setDisabledSiblings("true", 0, (wxWindow*)0);
    {
	...
	for(i=0; i<2; i++){ _bounds_grid->SetColSize(i, 35); }
...
void SPFrame::setBoundsGrid( var_map &V) //string &incs, string &excs ){
{
	...
	*/
	...
	for(unsigned int i=0; i<incs->size(); i++){	//For each polygon in the list
		//poly = split(i_polys.at(i), "[P]");	//Get a list of all of the points in the polygon
		...
		for(int j=0; j<np; j++){	//For each point in the polygon
	...
	//Now the exclusions
	for(unsigned int i=0; i<excs->size(); i++){	//For each polygon in the list
		//poly = split(e_polys.at(i), "[P]");	//Get a list of all of the points in the polygon
...
void SPFrame::UpdateDesignSelect( int sel, var_map &vset ){
	...
	*/
	...
	//check to see if any data has been entered in the grid
	/*
	if(_design_combo_panel->IsShown() ){
		wxString gtext;
		for(int i=0; i<_design_grid->GetNumberRows(); i++){
			for(int j=0; j<_design_grid->GetNumberCols(); j++){
				gtext.append( _design_grid->GetCellValue(i, j) );
			}
		}
		if( gtext != wxT("") ){
			//display a warning dialog
			wxMessageDialog *dlg = new wxMessageDialog(this, wxT("This action will affect the contents of the input grid. \nWould you like to continue?"), wxT("Warning"), wxYES_NO|wxICON_EXCLAMATION);
	...
	*/
	...
	if(sel == var_solarfield::DES_SIM_DETAIL::SUBSET_OF_DAYSHOURS){	//1) Subset of days/hours=2;
		...
		if(_design_nhours->IsShown() ){_design_nhours->Hide(); _design_nhours_lab->Hide(); }
		...
		day of month | month
		*/
		if(! _design_simallhrs->GetValue()){ //full grid
	...
	else if(sel == var_solarfield::DES_SIM_DETAIL::SINGLE_SIMULATION_POINT){	//2) Single design point=1;
	...
	else if(sel == var_solarfield::DES_SIM_DETAIL::DO_NOT_FILTER_HELIOSTATS){	//3) Do not filter heliostats=0;
		if( _design_combo_panel->IsShown() ) _design_combo_panel->Hide();
		if( _design_ndays->IsShown() ) {_design_ndays->Hide(); _design_ndays_lab->Hide();}
		if(_design_nhours->IsShown() ){_design_nhours->Hide(); _design_nhours_lab->Hide(); }
		if(_design_days_display->IsShown() ) _design_days_display->Hide();
		if(_design_helptext->IsShown()) _design_helptext->Hide();
	...
	else if(sel == var_solarfield::DES_SIM_DETAIL::ANNUAL_SIMULATION){	//4) Annual simulation=3;
		if( _design_combo_panel->IsShown() ) _design_combo_panel->Hide();
		if( _design_ndays->IsShown() ) {_design_ndays->Hide(); _design_ndays_lab->Hide();}
		if(_design_nhours->IsShown() ){_design_nhours->Hide(); _design_nhours_lab->Hide(); }
		if(_design_days_display->IsShown() ) _design_days_display->Hide();
	...
	else if(sel == var_solarfield::DES_SIM_DETAIL::LIMITED_ANNUAL_SIMULATION || sel == var_solarfield::DES_SIM_DETAIL::REPRESENTATIVE_PROFILES){	//5) Limited annual simulation=4 || Representative profiles=5
		if( _design_combo_panel->IsShown() ) _design_combo_panel->Hide();
		/*Show the input for the number of days to use*/
		if(! _design_ndays->IsShown() ) {_design_ndays->Show(); _design_ndays_lab->Show();}
		if(! _design_nhours->IsShown() ){_design_nhours->Show(); _design_nhours_lab->Show(); }
		if(! _design_days_display->IsShown() ) _design_days_display->Show();
		if(_design_helptext->IsShown()) _design_helptext->Hide();
		...
		//vector<string> tsdat = split(vset["solarfield"][0]["sim_step_data"].value, "[P]");
        WeatherData *tsdat = &vset.sf.sim_step_data.Val();
		int nsim = (int)tsdat->size();
		
		//Split each data entry into its individual components
		/*vector<vector<string> > alldat;
		for(int i=0; i<nsim; i++){
			alldat.push_back(vector<string>());
			alldat.back() = split(tsdat.at(i), ",");
		}*/
		...
		for(int i=1; i<nsim; i++){
			...
			//if( alldat.at(i).at(0) + alldat.at(i).at(2) != alldat.at(i-1).at(0) + alldat.at(i-1).at(2) ){
		...
		for(int i=0; i<nday; i++){
			...
			int hrcount=0;
			while(j<ranges.at(i)){
				//to_double(alldat.at(j).at(3), &dni);
                dni = tsdat->DNI.at(j);
			...
			//int month, dom, doy;
			//to_integer(alldat.at(ranges.at(i)-1).at(0), &dom);
			//to_integer(alldat.at(ranges.at(i)-1).at(2), &month);
...
void SPFrame::OnDesignEdit( wxGridEvent &WXUNUSED(event)){
	...
	*/
	try{
		...
		//string *data = &_variables["solarfield"][0]["sim_step_data"].value;
		//data->clear();

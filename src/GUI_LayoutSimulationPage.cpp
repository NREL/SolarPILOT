...
void SPFrame::CreateSimulationsLayoutTab( wxScrolledWindow *layout){
	...
	//_layout_button->GetToolTip()->SetDelay(_tool_tip_delay);
	...
	//_refresh_warning->GetToolTip()->SetDelay(_tool_tip_delay);
	...
	//Optimization setup
	//wxStaticBox *sb1 = new wxStaticBox(layout, wxID_ANY, wxT("Optimization setup"));
	//wxStaticBoxSizer *sbs1 = new wxStaticBoxSizer(sb1, wxVERTICAL);
	//layout_sizer->Add(sbs1, 0, wxALL, 10);
	...
	//_layout_delete->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( SPFrame::OnLayoutImport ), NULL, this);
	//_layout_insert->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( SPFrame::OnLayoutImport ), NULL, this);
	...
	//lay_but_sizer->Add((1,25),0,0,0);
	//lay_but_sizer->Add(lay_sl_2, 0, wxEXPAND|wxALL, 5);
	//lay_but_sizer->Add((1,25),0,0,0);
	//lay_but_sizer->Add(_layout_delete, 0, wxALL|wxALIGN_CENTER_VERTICAL, 5);
	//lay_but_sizer->Add(_layout_insert, 0, wxALL|wxALIGN_CENTER_VERTICAL, 5);
	/*lay_but_sizer->Add((1,25),0,0,0);
	lay_but_sizer->Add(lay_sl_3, 0, wxEXPAND|wxALL, 5);
	lay_but_sizer->Add((1,25),0,0,0);
	lay_but_sizer->Add(_refresh_warning, 0, wxALL|wxALIGN_CENTER_VERTICAL, 5);
	lay_but_sizer->Add(_layout_refresh, 0, wxALL|wxALIGN_CENTER_VERTICAL, 5);
	lay_but_sizer->Add(ref_label, 0, wxLEFT|wxALIGN_CENTER_VERTICAL, 5);*/
...
void SPFrame::UpdateLayoutGrid(){
	...
	//string *var = &_variables["solarfield"][0]["layout_data"].value;
	//var->clear();
	//wxString sdat, sdat2, sdat3, sdat4;
	...
	//save the layout to the variable map
	//interop::UpdateMapLayoutData(_variables, heliostats);
...
void SPFrame::SimProgressUpdate(simulation_info* siminfo){
	...
	*/
	...
	//Get the info from the solar field
	//simulation_info *sim = _SF.getSimInfoObject();
	...
	*/
	...
	try{
		...
		try
		{
			...
			_SF.Create(_variables);		if(_SF.ErrCheck()){goto sim_error_flag;}
...
void SPFrame::OnLayoutExport( wxCommandEvent &WXUNUSED(event)){
	...
	try{
		...
		if(filedlg.ShowModal() == wxID_OK)
		{
			...
			//write the header
			//H->getType(), loc->x, loc->y, loc->z, H->getFocalX(), H->getFocalY(), cant->i,cant->j, cant->k, aim->x, aim->y, aim->z);

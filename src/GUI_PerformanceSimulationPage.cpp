...
void SPFrame::CreateSimulationsFluxTab( wxScrolledWindow *flux){
	...
	//save_data_loc->GetToolTip()->SetDelay(_tool_tip_delay);
	...
	//_flux_button->GetToolTip()->SetDelay(_tool_tip_delay);
...
void SPFrame::UpdateFluxLC(int sortkey){
	...
	*/
	//Create a map between the template number and the name
	//std::map<string, string> temp_map;
	//for(unsigned int i=0; i<_heliotemp_info.size(); i++){
		//temp_map[ my_to_string(_heliotemp_info.at(i).index) ] = _heliotemp_info.at(i).name;
	//}
	...
	*/
	...
	for(int i=0; i<nh; i++){
		...
		if(sortkey == 0){ cdata.at(i) =  helios->at(i)->getSlantRange() ; }
		else if(sortkey == 1){ cdata.at(i) = helios->at(i)->getRadialPos(); }
		else if(sortkey == 2){ cdata.at(i) = helios->at(i)->getAzimuthalPos()*R2D; }
		else if(sortkey == 3){ cdata.at(i) = helios->at(i)->getLocation()->x; }
		else if(sortkey == 4){ cdata.at(i) = helios->at(i)->getLocation()->y; }
		else if(sortkey == 5){ cdata.at(i) = helios->at(i)->getAimPoint()->x; }
		else if(sortkey == 6){ cdata.at(i) = helios->at(i)->getAimPoint()->y; }
		else if(sortkey == 7){ cdata.at(i) = helios->at(i)->getAimPoint()->z; }
		else{ return; }
	}
	...
	for(int i=0; i<nh; i++){	//for each heliostat
		//Get the info
		H = _flcsort.at(i);

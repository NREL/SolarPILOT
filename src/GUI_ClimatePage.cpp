...
void SPFrame::CreateClimatePage(wxScrolledWindow *parent){
    ...
    //_variables.amb.atm_coefs.combo_select_by_choice_index(0);
	//_variables.amb.atm_coefs.val = _variables["ambient"][0]["atm_coefs"].choices.at(0);
	int ncoefs = _variables.amb.atm_coefs.val.ncols(); //atm_coef_defs.size();
...
void SPFrame::LoadClimateFiles(){
	...
	*/
	...
	for(int i=0; i<nwf; i++){
		if(wf_saved == _climate_files.at(i)){ isave=i; }
	}
...
void SPFrame::UpdateClimateFile( wxString &file,  var_map &V, bool mapval_only ){
	...
	for (int i=0;i<nrec;i++)
	{
		...
		//double gh, dn, df, wind, tdry, twet, relh, pres, wdir, snow;
        wf.read(&wrec);
		...
		//val->append("[P]"); val->append(line);	//Fully formatted data to variable map
...
void SPFrame::UpdateUserSunGrid(){
	...
	for(int i=0; i<nlines; i++){
		//vals = split(lines.at(i), ",");
		_user_sun_grid->SetCellValue(i,0, my_to_string(_variables.amb.user_sun.val.at(i,0)) ); //vals.at(0));
...
void SPFrame::OnSunImport( wxCommandEvent &WXUNUSED(event)){
	...
	*/
	...
	wxFileDialog filedlg(this, "Select a file for import...", wxEmptyString, wxEmptyString, "Sun shape files (*.sun;*.txt;*.dat;*.csv)|*.sun;*.txt;*.dat;*.csv" );
		...
		if(ioutil::file_exists( info.c_str() ) ){
			...
			*/
			...
			for(unsigned int i=0; i<delims.size(); i++){
				...
				if(data.size()>ns){ delim = delims.at(i); ns = data.size(); }	//pick the delimiter that returns the most entries
			}
			data.clear();
			...
			for(int i=0; i<nlines; i++){
				...
				//stemp.sprintf(fmt, fdat[0], fdat[1]);
...
void SPFrame::OnSunExport( wxCommandEvent &WXUNUSED(event)){
	...
	*/
	...
	//Get the file
	wxFileDialog filedlg(this, "Select a file for export...", wxEmptyString, wxEmptyString, "Sun shape files (*.sun;*.txt;*.dat;*.csv)|*.sun;*.txt;*.dat;*.csv", wxFD_SAVE|wxFD_OVERWRITE_PROMPT );
	filedlg.CentreOnParent();
...
void SPFrame::OnSunEdit( wxCommandEvent &WXUNUSED(event)){
	...
	//string *val = &_variables.amb.user_sun.val;
...
void SPFrame::OnAtmUpdate( wxCommandEvent &WXUNUSED(event)){
	...
	*/
	...
	if(nselect < 3){	//DELSOL3 clear day, DELSOL3 hazy day, User-defined
		...
		//vector<string> vals = split(_variables["ambient"][0]["atm_coefs"].choices.at(nselect), ",");
		for(int i=0; i<nord; i++){
			//_atm_coefs.at(i)->SetValue(vals.at(i));
		...
		//Update the variable map "value" to equal the current selection
		//_variables.amb.atm_coefs.val = _variables["ambient"][0]["atm_coefs"].choices.at(nselect);
...
void SPFrame::OnAttenFocus( wxFocusEvent &WXUNUSED(event)){
	...
	*/
	...
	//_variables["ambient"][0]["atm_coefs"].choices.at(2) = *var;

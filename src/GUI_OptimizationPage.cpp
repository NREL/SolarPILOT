...
void SPFrame::CreateOptimizationPage(wxScrolledWindow *parent)
{
	try{
		...
		//_optimize_button->GetToolTip()->SetDelay(_tool_tip_delay);
		...
		_optimize_detail_gauge = new wxGauge(parent, wxID_ANY, 1000, wxDefaultPosition, wxSize(200,25), wxGA_HORIZONTAL|wxGA_PROGRESS); //|wxGA_SMOOTH );
...
void SPFrame::OnApplyOptimizationResult( wxCommandEvent &WXUNUSED(evt))
{
	try{
		...
		if( _best_opt_result.size() > 0 )
        {
			...
			//vector<string> dat = split((string)_best_opt_result, ",");
            //vector<string> dat;
           ...
           /* _variables.sf.tht.set_from_string( dat[0].c_str() );
            _variables.recs[0].rec_aspect.set_from_string( dat[1].c_str() );
            _variables.recs[0].rec_height.set_from_string( dat[2].c_str() );
            _variables.land.max_scaled_rad.set_from_string( dat[3].c_str() );*/
            
			
			//also update the width and diameter of the receiver
            /*_variables.recs[0].rec_diameter.val = _variables.recs[0].rec_height.val / _variables.recs[0].rec_aspect.Val();
            _variables.recs[0].rec_width.val = _variables.recs[0].rec_diameter.val;*/
...
void SPFrame::OnOptimizeAddVar( wxCommandEvent &event)
{
	try{
		...
		if(dlg->ShowModal() == wxID_OK){
			...
			/*ArrayString wfiles;
			gui_util::list_files( _climate_dir, wfiles, "eps csv tm2 tm3 tmy");
			opt_data.SetWeatherFileList(wfiles);*/

			int nnew=0;
			for(int i=0; i<(int)vals.size(); i++){
				if(_opt_data.Index( vals[i].ToStdString() ) > -1) continue;	//If the item already exists, dont mess with it.
				_opt_data.addVar( _variables._varptrs.at(vals[i].ToStdString() ) ); // getVarByString(_variables, vals[i].ToStdString()) );
				nnew++;
			}

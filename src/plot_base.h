...
public:
	...
	//Virtual methods
	/*virtual bool Plot(vector<vector<double> > &data){return false;};
	virtual bool Export(wxString filename, const long type=wxBITMAP_TYPE_PNG){return false;};
	virtual bool ExportDataTable(wxString &path, wxString &delim, vector<vector<double> > &data){return false;};*/
};

...
string to_string(wxString &wxs){string s = (string)wxs; return s;}
string to_varpath(string &group, int &ID, string &name){return group + "." + my_to_string(ID) + "." + name;};
string to_varpath(string &group, string &ID, string &name){return group + "." + ID + "." + name;};
...
public:
    ...
    bool OnInit()
    {
	    ...
	    /*wxString all_vals;
	    for(int i=0; i<this->argv.GetArguments().size(); i++){
		    all_vals.Append(this->argv.GetArguments()[i]+"\n");
	    }*/
	    ...
	    for(int i=0; i<(int)dirfiles.size(); i++){
		    //if( dirfiles.at(i).find("~") != std::string::npos){
    ...
    wxFrame(parent, id, title, pos, size, style)
{
    ...
    /*
    int xpx = wxSystemSettings::GetMetric(wxSYS_SCREEN_X);
    int ypx = wxSystemSettings::GetMetric(wxSYS_SCREEN_Y);
    int xw = (int)(0.7 * xpx);
    int yw = (int)(0.5 * ypx);

    wxString url = "C:/Users/mwagner/Documents/NREL/Dev/solarpilot/help/texhelp/sample-webpage.html";
    
    wxFrame *pop = new wxFrame(0, wxID_ANY, "HELP", wxPoint((xpx-xw)/2, (ypx-yw)/2), wxSize(xw,yw) ); //, wxCLOSE_BOX | wxCLIP_CHILDREN | wxRESIZE_BORDER );
    wxWebView *html = wxWebView::New(pop, wxID_ANY, url);
    html->Connect( wxEVT_KEY_DOWN, wxKeyEventHandler( SPFrame::TempEscape ), NULL, this);

    wxBoxSizer *popsize = new wxBoxSizer(wxVERTICAL);
    popsize->Add(html, 1, wxEXPAND, 0);
    pop->SetSizer(popsize);
    pop->Layout();
    pop->Show();


    wxBoxSizer *main_sizer = new wxBoxSizer(wxVERTICAL);
    //main_sizer->Add(html, 1, wxEXPAND, 0);
    ...
    */
	...
	//Set the version tag
	_software_version = "2017.11.22-dev";
...
//void SPFrame::OnClose( wxCommandEvent &event){
//    //Catch window X close event
//    //OnQuit(event);
//
//	if(! ModCheckPrompt() ) return;	//prompt to save. if the user cancels, return.
//
//	::wxExit();
//
//    event.Skip(true);
...
//void SPFrame::OnQuit(wxCommandEvent &evt ){
//	//check for save
//	if(! ModCheckPrompt() ) return;	//prompt to save. if the user cancels, return.
//
//	::wxExit();
...
void SPFrame::Save(int save_config){
	...
	*/
	...
	wxFileName file_use;
	bool is_prompt = true;
	if(save_config == 0){	//Use the currently opened file.
...
void SPFrame::Open(wxString file_in, bool quiet){
	...
	*/
	...
	if(isxml){
		...
		//ioutil::parseInputFile(file.ToStdString(), _variables, _definitions);	//Load in all of the variables for the interface
	}
	...
	//Notify the user
	//if(!quiet) PopMessage("File successfully opened!", ""); 
...
void SPFrame::AddRecentFile(wxFileName &fname){
	...
	for(int i=1; i<nvals; i++){
		if( lower_case(fname.GetFullPath().ToStdString()) == lower_case(fvals.at(i)) ){ fvals.erase(fvals.begin()+i); nvals--;}
	}
...
void SPFrame::OnHelpContents( wxCommandEvent &WXUNUSED(event) ){
	...
	//PopMessage("Sorry - no help contents are currently available.");

	ShowContextHelp();

	
	//wxLaunchDefaultApplication( _install_dir + "exelib\\help\\SolarPILOT.chm");
...
void SPFrame::bindControls(){
	...
	for( iti=_input_map.begin(); iti != _input_map.end(); iti++ ){
		...
		if(iti->second->isCombo()){
			...
			//iti->second->Connect(wxEVT_COMMAND_TEXT_UPDATED, wxCommandEventHandler( SPFrame::OnTextEvent ), NULL, this);
		}
	}
}
...
void SPFrame::UpdateInputValues(){
	...
	//Climate file
	//wxString wname = _variables["ambient"][0]["weather_file"].value;
		...
		if(it->second->isCombo() || it->second->isCheckbox()){
			...
			if(disobj->size() > 0){
				...
				for(unsigned int i=0; i<disobj->size(); i++){
					//if(_input_map.find( dis->at(i) ) != _input_map.end()){
						//Disable the control that's in the input map
						//if(_input_map.at(dis->at(i))->IsEnabled()) _input_map.at(dis->at(i))->Disable();
						odisplay[ disobj->at(i) ] = false;
					//}
					//else if(_output_map.find( dis->at(i) ) != _output_map.end()){
						//Disable the control that's in the output map
						//if(_output_map.at(dis->at(i))->IsEnabled()) _output_map.at(dis->at(i))->Disable();
						//odisplay[ _output_map.at( dis->at(i) ) ] = false;
					//}
				}
			}
			if(disobj->size() > 0){
				...
				for(unsigned int i=0; i<disobj->size(); i++){
					//Disable the control
					//if(disobj->at(i)->IsEnabled()) disobj->at(i)->Disable();
					odisplay[ disobj->at(i) ] = false;
				}
			}

			//Apply highlighting        >> Disable for now. It's a little confusing
			//if(it->second->isCombo()){
			//	//Get the highlighted siblings
			//	highs = it->second->getHighlightSiblings( it->second->getSelectedText() ); 
			//}
			//else{
			//	string val = it->second->getValue();
			//	highs = it->second->getHighlightSiblings( lower_case(val) );
			//};
			//if(highs->size() > 0){
			//	for(unsigned int i=0; i<highs->size(); i++){
			//		if(_input_map.find( highs->at(i) ) != _input_map.end()){
			//			_input_map.at(highs->at(i))->setColour( *it->second->getHighlightColour() );
			//		}
			//	}
			//}
		}
...
bool SPFrame::GetGeomState(){ return _geom_modified; }

//void SPFrame::DoLayoutPlot(bool has_data){
//	/* 
//	reusable method for creating the layout plot page 
//	"has_data" indicates whether any simulation data is available for plotting
//	*/
//
...
//	}
//	wxScrolledWindow *page = new wxScrolledWindow(this);
//	CreateFieldPlotPage(page, choices, has_data ? 2 : 0);
//    _page_panel->InsertPage( _page_panel->GetPagePosition( pageNames.results )+1, page, pageNames.results_layout, ID_ICON_LAYOUT, 1, pageNames.results);
//	_main_sizer->Add( page, 1, wxEXPAND, 0);
//    //Also call the flux plotter to keep things in the right order
//	//DoFluxPlot();
//
//}

//void SPFrame::DoFluxPlot(){
//	/* 
//	reusable method for creating the flux plot page 
//	*/
//
//    _page_panel->DestroyPage( pageNames.results_flux );
//
//	wxScrolledWindow *page = new wxScrolledWindow(this);
//	CreateFluxPlotPage(page);
//    _page_panel->InsertPage( _page_panel->GetPagePosition( pageNames.results_layout )+1, page, pageNames.results_flux, ID_ICON_FLUX, 1, pageNames.results );
//	_main_sizer->Add( page, 1, wxEXPAND, 0);
//}

void SPFrame::DoResultsPage(sim_results &results){
	//Delete old pages
	//_page_panel->DestroyPage( pageNames.results_summary );
...
void SPFrame::SimProgressBase(int n_complete, int n_tot, string *notices, wxGauge *active_gauge, wxStaticText *active_label, wxString label_text){
	...
	double simprog;
	//wxString simstr;
	//simstr.Printf("%s (%d/%d)", label_text, n_complete+1, n_tot);
	//if(simstr != active_label->GetLabel())	
...
void SPFrame::FluxProgressBase(int n_complete, int n_tot, wxGauge *active_gauge, wxStaticText *active_label, wxString label_text){
	...
	double simprog;
	//wxString simstr;
	//simstr.Printf("%s (%d/%d)", label_text, n_complete, n_tot);
	//if(simstr != active_label->GetLabel())	
...
int SPFrame::SolTraceProgressUpdate(st_uint_t ntracedtotal, st_uint_t ntraced, st_uint_t ntotrace, st_uint_t curstage, st_uint_t nstages, void *data)
{
	...
	*/
	...
	if(_cancel_simulation) return 0;	//Code for coretrace to quit
	
	//Update the gui
	//wxString simstr;
	double pfrac = (double)ntraced/max((double)ntotrace, 1.)*100.;
	int fprog;
	if(pfrac>100. || pfrac < 1.e-6) fprog = 0;
	else fprog = (int)pfrac;
	...
	//simstr = wxString::Format("Progress (Stage %d/%d): %.2f%s", (int)curstage, (int)nstages, pfrac,"%");
...
bool SPFrame::SolTraceFluxBinning(SolarField &SF)
{
   //Collect all of the rays that hit the receiver(s) into the flux profile
    int rstage1 = 2;
	/*unordered_map<int, Receiver*> rstage_map;
	for(unsigned int i=0; i<SF.getReceivers()->size(); i++){
		if(SF.getReceivers()->at(i)->isReceiverEnabled()) rstage_map[i] = SF.getReceivers()->at(i);
	}*/

	//for(unordered_map<int, Receiver*>::iterator it = rstage_map.begin(); it != rstage_map.end(); it++){
    for(int r = 0; r<(int)SF.getReceivers()->size(); r++)
    {
		...
		case Receiver::REC_GEOM_TYPE::CYLINDRICAL_CLOSED:		//0	|	Continuous closed cylinder - external
        {
			...
			for(int j=0; j<_STSim->IntData.nint; j++){	//loop through each intersection
	...
	*/
    ...
    if( ! is_sam_grid_auto )
    {
	    ...
	    */
	    ...
	    wxString pline;
	    for(int i=0; i<neff_az; i++){ pline.Append( wxString::Format("%7.2f,", eff_az[i]) ); }
	    pline.Append( wxString::Format("%7.2f,", 360.));
	    file.AddLine( pline );
	    pline.Clear();
	    for(int i=0; i<neff_zen; i++){ pline.Append( wxString::Format("%7.2f,", eff_zen[i])); }
	    file.AddLine( pline );
	...
	if(fluxmap_format == 0 || fluxmap_format == 1){	//SAM format || 12 x 10 (same thing)
...
void SPFrame::ParametricSimulate( parametric &P ){
	...
	*/
	...
	while(! sim_complete){
		//wxLogMessage((wxString)("Beginning simulation "+my_to_string(nsim)));
		...
		for(int i=0; i<(int)linked.size(); i++)
        {
			...
			//getVarByString(vset, linked.at(i)->varname)->value = value;
		...
		for(int i=0; i<(int)unlinked.size(); i++){
			...
			//getVarByString(vset, unlinked.at(i)->varname)->value = value;
		...
		if(any_linked && parchanged[0]){	//linked values
			for(int i=0; i<(int)linked.size(); i++){
		...
		try{
			...
			if(varpar.flux.flux_solar_el.Val() < 0.){
				...
				try{
					...
					_par_SF.Clean();
					_par_SF.Create(varpar);
					//wxLogMessage((wxString)("SolarField object created "+my_to_string(nsim)));
...
void SPFrame::CreateResultsTable(sim_result &result, grid_emulator &table){
	try{
		...
		table.SetColLabelValue(3, "Maximum");
		table.SetColLabelValue(4, "Std. dev");
...
void SPFrame::OnComboEvent( wxCommandEvent &event){
	try{
		...
		//special handlers
		/*wxComboBox *obj = ( (wxComboBox*)event.GetEventObject() );
		string varpath = ( (InputControl*)obj->GetParent() )->getVarPath();
		if(varpath == "solarfield.0.des_sim_detail") 
        {
            
			UpdateDesignSelect(_variables.sf.des_sim_detail.val, _variables);
		}*/
		event.Skip();
	}
	catch( std::exception &e )
